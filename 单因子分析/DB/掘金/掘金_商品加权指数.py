...
if __name__ == '__main__':
    ...
    for code in constant.CODE_LIST:
        ...
        # 获取品种指数合约代码 【交易所.大写品种代码 99】
        symbol = utils.getSymbol(code)

        # 指定日期区间
        singleDataRaw = get_history_symbol(symbol=symbol, start_date=constant.startStr, end_date=constant.endStr,
                                           df=True)
        # 格式化存储
        ...
        singleDataRaw.rename(columns={"trade_date": "date", "pre_close": "close"}, inplace=True)
    ...
    df.to_csv("../CSV导出/" + "掘金_商品加权指数" + constant.startStr + "TO" + constant.endStr + ".csv")
